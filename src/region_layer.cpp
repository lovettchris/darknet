...
void delta_region_class(float *output, float *delta, int index, int _class, int classes, tree *hier, float scale, int stride, float *avg_cat)
{
    int i, n;
    if(hier){
        float pred = 1;
        while(_class >= 0){
            ...
            delta[index + stride*_class] = scale * (1 - output[index + stride*_class]);

			_class = hier->parent[_class];
        }
        *avg_cat += pred;
    } else {
        for(n = 0; n < classes; ++n){
            delta[index + stride*n] = scale * (((n == _class)?1 : 0) - output[index + stride*n]);
            if(n == _class) *avg_cat += output[index + stride*n];
        }
    }
}
    ...
    for (b = 0; b < l.batch; ++b) {
        if(l.softmax_tree){
            ...
            for(t = 0; t < 30; ++t){
                ...
                if(!truth.x) break;
                int _class = net.truth[t*5 + b*l.truths + 4];
                float maxp = 0;
                int maxi = 0;
                if(truth.x > 100000 && truth.y > 100000){
                    for(n = 0; n < l.n*l.w*l.h; ++n){
                        ...
                        float p = scale*get_hierarchy_probability(l.output + class_index, l.softmax_tree, _class, l.w*l.h);
                        if(p > maxp){
                            maxp = p;
                            maxi = n;
                        }
                    ...
                    delta_region_class(l.output, l.delta, class_index, _class, l.classes, l.softmax_tree, l.class_scale, l.w*l.h, &avg_cat);
