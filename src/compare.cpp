...
int bbox_comparator(const void *a, const void *b)
{
    ...
    free_image(im2);
    free(X);
    if (predictions[_class *2] > predictions[_class *2+1]){
        return 1;
    }
    return -1;
}

void bbox_update(sortable_bbox *a, sortable_bbox *b, int _class, int result)
{
    int k = 32;
    float EA = 1./(1+pow(10, (b->elos[_class] - a->elos[_class])/400.));
    float EB = 1./(1+pow(10, (a->elos[_class] - b->elos[_class])/400.));
    float SA = result ? 1 : 0;
    float SB = result ? 0 : 1;
    a->elos[_class] += k*(SA - EA);
    b->elos[_class] += k*(SB - EB);
}

void bbox_fight(network net, sortable_bbox *a, sortable_bbox *b, int classes, int _class)
{
...
void BattleRoyaleWithCheese(char *filename, char *weightfile)
{
    ...
    for (_class = 0; _class < classes; ++_class){
        ...
        for(round = 1; round <= 100; ++round){
            ...
            for(i = 0; i < N/2; ++i){
                bbox_fight(net, boxes+i*2, boxes+i*2+1, classes, _class);
            }
            qsort(boxes, N, sizeof(sortable_bbox), elo_comparator);
        ...
        char buff[256];
        sprintf(buff, "results/battle_%d.log", _class);
        FILE *outfp = fopen(buff, "w");
        for(i = 0; i < N; ++i){
            fprintf(outfp, "%s %f\n", boxes[i].filename, boxes[i].elos[_class]);
        }
        fclose(outfp);
    }
